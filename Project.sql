-- Data Cleaning

SELECT *
FROM layoffs;

CREATE TABLE layoffs_replica
LIKE layoffs;

SELECT *
FROM layoffs_replica;

INSERT layoffs_replica
SELECT *
FROM layoffs;

SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`, stage, country, funds_raised_millions) AS row_num
FROM layoffs_replica;

WITH duplicate_cte AS
( SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`) AS row_num
FROM layoffs_replica
)
SELECT *
FROM duplicate_cte
WHERE row_num > 1;

SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`, stage, country, funds_raised_millions) AS row_num
FROM layoffs_replica;

WITH duplicate_cte AS
( SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`, stage, country, funds_raised_millions) AS row_num
FROM layoffs_replica
)
SELECT *
FROM duplicate_cte
WHERE row_num > 1;

SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`, stage, country, funds_raised_millions) AS row_num
FROM layoffs_replica


CREATE TABLE `layoffs_replica2` (
  `company` text,
  `location` text,
  `industry` text,
  `total_laid_off` int DEFAULT NULL,
  `percentage_laid_off` text,
  `date` text,
  `stage` text,
  `country` text,
  `funds_raised_millions` int DEFAULT NULL,
  `row_num` INT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;


SELECT *
FROM layoffs_replica2

INSERT INTO layoffs_replica2
SELECT *,
ROW_NUMBER() OVER(PARTITION BY company, industry, total_laid_off, percentage_laid_off, `date`, stage, country, funds_raised_millions) AS row_num
FROM layoffs_replica;


SELECT *
FROM layoffs_replica2
WHERE row_num >1;


DELETE
FROM layoffs_replica2
WHERE row_num >1;

SELECT*
FROM layoffs_replica2;


-- STANDARDIZING DATA

SELECT company, TRIM(company)
FROM layoffs_replica2;

UPDATE layoffs_replica2
SET company = trim(company);

SELECT *
FROM layoffs_replica2
WHERE industry LIKE 'Crypto%';


UPDATE layoffs_replica2
SET industry = 'Crypto'
WHERE industry LIKE 'Crypto%';

SELECT DISTINCT industry
FROM layoffs_replica2;

SELECT DISTINCT location
FROM layoffs_replica2;

SELECT DISTINCT country
FROM layoffs_replica2
ORDER BY 1;

SELECT `date`,
STR_TO_DATE(`date`, '%m/%d/%Y')
FROM layoffs_replica2;

UPDATE layoffs_replica2
SET country = 'United States'
WHERE country LIKE 'United States%';



UPDATE layoffs_replica2
SET `date` = STR_TO_DATE(`date`, '%m/%d/%Y');

ALTER TABLE layoffs_replica2
MODIFY COLUMN `date` DATE;

SELECT *
FROM layoffs_replica2
WHERE total_laid_off IS NULL
AND percentage_laid_off IS NULL;

UPDATE layoffs_replica2
SET industry = NULL
where industry = '';

SELECT *
FROM layoffs_replica2
WHERE industry IS NULL
OR industry = '';

SELECT *
FROM layoffs_replica2
WHERE company = "Bally's Interactive";

SELECT t1.industry, t2.industry
FROM layoffs_replica2 t1
JOIN layoffs_replica2 t2
 ON t1.company=t2.company
 AND t1.location=t2.location
WHERE (t1.industry IS NULL or t1.industry = '')
AND t2.industry IS NOT NULL;

UPDATE layoffs_replica2 t1
JOIN layoffs_replica2 t2
 ON t1.company=t2.company
SET t1.industry = t2.industry
WHERE (t1.industry IS NULL or t1.industry = '')
AND t2.industry IS NOT NULL;

SELECT *
FROM layoffs_replica2
WHERE total_laid_off IS NULL
AND percentage_laid_off IS NULL;


DELETE
FROM layoffs_replica2
WHERE total_laid_off IS NULL
AND percentage_laid_off IS NULL;

SELECT *
FROM layoffs_replica2;

ALTER TABLE layoffs_replica2
DROP COLUMN row_num;